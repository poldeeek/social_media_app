...
const {
  isUserExistIdParams,
  isUserExistIdBody,
} = require("../../middleware/paramsValidations/isUserExist");
const {
  isPostExist,
} = require("../../middleware/paramsValidations/isPostExist");
const {
  isCommentExist,
} = require("../../middleware/paramsValidations/isCommentExist");
...
router.get("/getMessages/:chatId", accessTokenVerify, (req, res) => {
  ...
  Message.find(options)
    ...
    .catch((err) => {
      return res
        .status(500)
        .json({ error: "Database getting messages problem." });
    });
});
...
router.post("/seeMessages/:id", accessTokenVerify ,isUserExistIdBody, (req, res) => {
    ...
    Message.updateMany(
      { chat_id: chatId, author_id: { $ne: userId } },
      {
        seen: true,
      }
    )
      .then((result) => res.status(200).json({ msg: "Messages set as seen." }))
      .catch((err) =>
        res.status(500).json({ msg: "Database see messages problem." })
      );
  }
);
